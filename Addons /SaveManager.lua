...
local SaveManager = {} do
    ...
    SaveManager:BuildFolderTree()
    self:ApplyLunarStyle()
end

function SaveManager:ApplyLunarStyle()
    if not self.Library then return end

    local accent = self.Library.AccentColor
    local main = self.Library.MainColor
    local outline = self.Library.OutlineColor
    local font = self.Library.FontColor

    -- Button styling
    for _, obj in pairs(self.Library.Buttons) do
        obj.TextColor = font
        obj.BackgroundColor = main
        obj.OutlineColor = outline
        obj.HoverColor = accent
    end

    -- Input styling
    for _, obj in pairs(self.Library.Options) do
        if obj.Type == "Input" then
            obj.Textbox.BackgroundColor3 = main
            obj.Textbox.TextColor3 = font
            obj.Textbox.PlaceholderColor3 = font:lerp(Color3.new(0,0,0), 0.5)
        end
    end
